...
import {updateAuditCaseDetails,getAllAuditCase} from "../actions/review";
...
const AfterVerification=(props)=>{
    ...
    useEffect(()=>{
        dispatch(getAllCaseAPI({id:'all',status: ['under_employee','verify_by_admin']}));
    },[])
...
updateRecords.forEach((record)=>{
    ...
    // dispatch(updateAuditCaseDetails(record))
    // .then(()=>{
    ...
    const handleUpdateSaveButton =(updateRadio)=>{
        let update  =verifyAdmin.find((ele)=>ele.id===updateRadio.id );
        setShowModal(updateRadio);
    }
    ...
    const handleUnreviewEditForm=(data)=>{
        ...
        // console.log("audit data",{id:'all',status: ['verify_by_admin'],audit_case_status_id:verifyAdmin.audit_case_status_id})
        // dispatch(getAllAuditCase({id:'all',status: ['verify_by_admin']}))
    }
    ...
    return(
        <Container  >
           ...
           <div> 
            <h2 style={{align:"center"}}>Calling & Review Verification List  
             &nbsp;  &nbsp;  &nbsp;
            <Button className="btn btn-primary" style={{align:"rigth"}} 
            onClick={actionButton ? handleViewVerify : handleReview }> {actionButton ? 'View Verified By Admin':'View Unreview' }
            </Button> </h2>  </div>
    ...
    viewButton={actionButton ? handleUpdateForm: handleUnreviewUpdateForm}
    />
    </div>
<div>
    {/* status under_employee  */}
    <ModalAfterVerify show={showModal} close={handleClose} type={modalType} defaultData={defaultData} onUpdate={handleUpdateSaveButton}
    onSave={ handleEditSave} />
    
</div>
<div>
    {/* under_admin */}
    <ModalAfterUnview open={showUnview} onClose={handleCloseUn} type={modalUnview} defaultData ={defaultData} 
